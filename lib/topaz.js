 ...
 *
 * @example
 * import { TopazPad } from 'https://l1n.github.io/topaz-dext/lib/topaz.js';
 *
...
export class TopazPad {
  constructor(model = 'SignatureGem1X5') {
  ...
  _decodePacket(buf, fmt) {
    ...
    // Point filter
    this._filterX.push(x); this._filterY.push(y);
    if (this._filterX.length > m.filter) { this._filterX.shift(); this._filterY.shift(); }
    const fx = this._filterX.reduce((a, b) => a + b, 0) / this._filterX.length;
    const fy = this._filterY.reduce((a, b) => a + b, 0) / this._filterY.length;

    const point = { x: fx, y: fy, p: this.pressure };
